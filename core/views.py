...
def ws_eventos(request):
    eventos = [evento.as_dict() for evento in Evento.objects.filter(fecha_inicio__date=date.today())]
    eventos += [evento.as_dict() for evento in Evento.objects.filter(importante=True, fecha_inicio__date__gt=date.today())]
